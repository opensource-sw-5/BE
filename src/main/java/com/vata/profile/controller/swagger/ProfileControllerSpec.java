...
public interface ProfileControllerSpec {
    ...
    @ApiResponse(responseCode = "200", description = "크레딧 잔액 조회 성공",
            content = @Content(schema = @Schema(implementation = StabilityCreditsResponse.class)))
    // 200 응답 시 반환될 DTO 명시
    @ApiResponse(responseCode = "401", description = "인증 실패 (유효하지 않은 Access Token 또는 로그인 필요)", content = @Content)
    // 401 응답 명시
    @ApiResponse(responseCode = "500", description = "서버 오류 또는 크레딧 조회 실패", content = @Content) // 500 응답 명시
    @GetMapping("/credits")
        // 실제 엔드포인트 경로를 인터페이스 메서드에 명시
